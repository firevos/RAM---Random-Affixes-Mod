...
{
    internal class ItemActionEntryUpgradeAffix : BaseItemActionEntry
    {
        public ItemActionEntryUpgradeAffix(XUiController controller) : base(controller, "lblContextActionUpgradeAffix", "ui_game_symbol_upgrade", BaseItemActionEntry.GamepadShortCut.None, "crafting/craft_click_craft", "ui/ui_denied") { }
                ...
                if (value < 6)
                {
                    ...
                    if (AffixSystem.UpgradeAffix(parentItemValue, upgradeSlot, ref affixName))
                    {
                            ...
                            parentItemValue.TryGetMetadata("lastUpgrade", out float lastUpgrade))
                        {
                            ...
                            int killsToUpgrade = AffixUtils.GetAdjustedKillsToUpgrade(magicSlayerLvl);
                            parentItemStack.itemValue.SetMetadata("upgrades", upgrades + 1);
                            parentItemStack.itemValue.SetMetadata("nextUpgrade", lastUpgrade + killsToUpgrade * (upgrades + 2));
                            this.ItemController.xui.AssembleItem.currentItem = parentItemStack;
                            this.ItemController.xui.AssembleItem.RefreshAssembleItem();
                        }
